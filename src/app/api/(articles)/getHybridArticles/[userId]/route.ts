...
const fetchUserPreferences = async (userId: string) => {
  ...
  const parsedCurrentUser: UserPreferenceType = JSON.parse(JSON.stringify(currentUser));

  const parsedOtherUsers: UserPreferenceType[] = JSON.parse(JSON.stringify(otherUsers));
...
const fetchArticlesByCategories = async (recommendations: CategorialCountType[]) => {
...
export async function GET(request: NextRequest, props: { params: Promise<{ userId: string }> }) {
  try {
    ...
    if (!baseTarget || isNaN(Number(baseTarget))) {
      return NextResponse.json({ error: 'Invalid or missing BaseTarget' }, { status: 400 });
    }
