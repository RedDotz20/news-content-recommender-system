...
export function calRemaining(
  ...
  for (let i = 0; i < currentGroup.length - 1; i++) {
    const randomIndexDestination = Math.floor(Math.random() * currentGroup.length);

    // Generate a random number that doesn't exceed the remaining value
    const randomNumber = Math.min(Math.floor(Math.random() * remainingValue) + 1, remainingValue);
  ...
  return currentGroup.filter((item: any) => item.articles > 0) as CategorialPrefWithArticlesType[];
}
