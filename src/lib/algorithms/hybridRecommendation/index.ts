...
export function hybridRecommendation(
  ...
  const contentBasedFiltered: ReturnType<typeof contentBasedFiltering> = contentBasedFiltering(
    targetUserCategorialPref,
    baseTarget
  );

  const collaborativeFiltered: ReturnType<typeof collaborativeFiltering> = collaborativeFiltering(
  ...
  if (collabFilteredMed) {
    ...
    const collabLowBracket = calRemaining(baseTarget * 0.1, collaborativeMedian.lowestBracket);
    ...
    const totalArticles = mergedArticles.reduce((sum, curr) => sum + curr.articles, 0);
    ...
    let currentTotal = distributedArticles.reduce((sum, curr) => sum + curr.articles, 0);
    ...
    const hybridRecommendedArticles: CategorialCountType[] = distributedArticles.reduce(
      (acc: CategorialCountType[], curr) => {
