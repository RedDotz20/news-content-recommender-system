...
describe('medDist', () => {
  ...
  test('happy path', () => {
    ...
    expect(result).toEqual([{ articles: 10 }, { articles: 10 }, { articles: 10 }]);
  });
  ...
  test('edge case: negative base target', () => {
    const baseTarget = -100;
    const medianArr: Partial<CategorialPrefWithArticlesType>[] = [{ articles: 0 }, { articles: 0 }];
  ...
  test('edge case: single element in median array', () => {
    const baseTarget = 50;
    const medianArr: Partial<CategorialPrefWithArticlesType>[] = [{ articles: 0 }];
