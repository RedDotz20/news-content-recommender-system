...
describe('calcMedianPreference', () => {
  test('should return correct median for odd number of preferences (7)', () => {
    ...
    const result: LowerAndMedianPrefsType = calcMedianPreference(0, preferences);
  ...
  test('should return correct medians for more than 15 preferences', () => {
    const preferences: CategorialPreferenceType[] = Array.from({ length: 16 }, (_, i) => ({
  ...
  test('should throw error for empty preferences array', () => {
    expect(() => calcMedianPreference(0, [])).toThrow('Preferences array cannot be empty');
  });
});
