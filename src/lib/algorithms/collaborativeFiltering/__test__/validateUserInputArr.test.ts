...
describe('validateUsersInputArr', () => {
	...
	test('valid input should not throw an error', () => {
		expect(() =>
			validateUsersInputArr(validUser, validUsersList)
		).not.toThrow();
	});
	...
	test('targetUser preferences is not a valid array', () => {
		expect(() =>
			validateUsersInputArr(
				{ userId: 'user1', preferences: [] } as any,
				validUsersList
			)
		).toThrow(
		...
		expect(() =>
			validateUsersInputArr(
				{ userId: 'user1', preferences: null } as any,
				validUsersList
			)
		).toThrow(
			'userAPreferences.preferences must be a non-null, non-empty array of CategorialPreferenceType'
	...
	test('one of the userBPreferences is not an object', () => {
		expect(() =>
			validateUsersInputArr(validUser, [null as any, ...validUsersList])
		).toThrow('Each userBPreference must be a valid UserPreferenceType object');

		expect(() =>
			validateUsersInputArr(validUser, [123 as any, ...validUsersList])
		).toThrow('Each userBPreference must be a valid UserPreferenceType object');
	});
