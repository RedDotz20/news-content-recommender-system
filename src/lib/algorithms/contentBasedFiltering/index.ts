...
export function contentBasedFiltering(
  ...
  const isValidInput =
    Array.isArray(inputArray) && inputArray.length >= 3 && inputArray.every(isValidItem);
  ...
  const calculateHalflength = (): number => {
    if (sortedPreferences.length < 3) {
      const errorMessage = 'The sortedPreferences array must have at least three items.';
  ...
  const reducedCategoryLength = sortedPreferences.slice(0, halfLength as number);

  const distributedArticles = pctContentDist('articleDistribution', reducedBaseTarget);

  const distributedCategories = pctContentDist('categoryIdentifier', reducedCategoryLength.length);

  return calculateArticleValues(reducedCategoryLength, distributedArticles, distributedCategories);
}
