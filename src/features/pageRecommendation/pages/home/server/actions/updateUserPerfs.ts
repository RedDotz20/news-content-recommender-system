...
export const updateUserPerfs = async (
	userId: string,
	preferences: mutatePreferenceType
) => {
	...
	try {
		...
		throw new Error(
			'Could not update user preferences. Please try again later.'
		);
	}
};

// export const updateUserPerfs = actionClient
// 	.schema(mutatePreferenceSchema)
// 	.action(async ({ parsedInput: { userId, preferences } }) => {
// 		const baseUrl = process.env.NEXT_PUBLIC_SITE_URL as string;
// 		try {
// 			const response = await fetch(
// 				`${baseUrl}/api/updatePrefsArray/${userId}`,
// 				{
// 					method: 'PUT',
// 					headers: {
// 						'Content-Type': 'application/json',
